...
class InterfaceController:
    ...
    def print_cursor(self):
        is_cell_under = self.board.is_alive(
            self.y_cursor - 1, self.x_cursor - 1
        )
    ...
    def move_cursor(self, direction: int):
        ...
        self.x_cursor = ensure_in_range(
            x_cursor, 1, self.screen.width // 2 - 2
        )
    ...
    def invert_cell(self):
        self.board.toggle_cell(
            self.y_cursor - self.board_position.y,
            self.x_cursor - self.board_position.x,
        )
    ...
    def print_pause(self):
        ...
        self.print_at(
            text, self.screen.width // 2 + 2, self.screen.height // 2
        )
